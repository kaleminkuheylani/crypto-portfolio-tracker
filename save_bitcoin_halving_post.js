// Script to save Bitcoin Halving Controversy post to Mongoose
...
async function saveToMongodb() {
  ...
  try {
    ...
    const database = client.db('kriptosavasi');
    const collection = database.collection('blogs');
    
    // Check if post already exists
    const existingPost = await collection.findOne({ slug: bitcoinHalvingContent.slug });
    if (existingPost) {
      console.log('Post already exists in MongoDB');
      return existingPost;
    }
    
    // Add additional fields for Mongoose compatibility
    const postData = {
    ...
    console.log('Successfully saved to MongoDB with ID:', result.insertedId);
...
// Run the script
console.log('Saving Bitcoin Halving Controversy post to MongoDB...');
saveToMongodb().then(() => {
